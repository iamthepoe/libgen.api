...
export class LibraryGenesisApi {
  ...
  constructor(
    htmlClient: typeof cheerio = cheerio,
  ...
  ): Promise<LibraryGenesisResponse<Book[]>> {
  ...
  private parseBooksFromHtml(html: string): Book[] {
    ...
    const rows = table
      .find('tr')
      .filter((_i, el) => $(el).find('td').length > 0)
      .slice(1);
  ...
  private async downloadBook(
    mirrorUrl: string
  ): Promise<LibraryGenesisResponse<Buffer>> {
    ...
    const fileResponse = await safeAsync(() =>
      this.httpClient.get(downloadLinkResult.data)
    );
  ...
  private getDownloadLinkbyFirstMirrorHtml(
    html: string
  ): LibraryGenesisResponse<string> {
    const $ = this.htmlClient.load(html);
    const downloadLink = $('a')
      .filter((_i, el) => $(el).text().includes('GET'))
      .attr('href');
